...
> Примечание: старый аргумент `-c` / `-console` **не используется** текущим CLI. Используйте подкоманды (`scan`, `mask`, `unmask`, `settings`).

### Команды

- `scan`: запуск сканирования и генерация отчёта
- `mask`: создание маскированного XLSX и обратимого mapping JSON
- `unmask`: восстановление XLSX по маскированному файлу и mapping JSON
...
## `mask` и `unmask` — обратимое маскирование XLSX

`mask` сканирует `.xlsx` файл выбранными матчерами, заменяет маскируемые персональные данные на стабильные токены и сохраняет mapping JSON. Mapping JSON нужно хранить приватно: в нём лежат исходные значения для восстановления.

### Параметры `mask`

| Параметр | Короткий | Значение | Описание |
|----------|----------|----------|----------|
| `--input` | `-i` | `file.xlsx` | Исходный XLSX файл. **Обязательно**. |
| `--out` | `-o` | `file.xlsx` | Путь для маскированного XLSX. **Обязательно**. |
| `--map` | - | `mapping.json` | Путь для mapping JSON. **Обязательно**. |
| `--matchers` | `-m` | `m1,m2,...` | Матчеры для поиска. По умолчанию: из настроек. |
| `--user-signatures` | `-us` | `s1,s2,...` | Существующие пользовательские сигнатуры. По умолчанию: из настроек. |

### Параметры `unmask`

| Параметр | Короткий | Значение | Описание |
|----------|----------|----------|----------|
| `--input` | `-i` | `file.xlsx` | Маскированный XLSX файл. **Обязательно**. |
| `--out` | `-o` | `file.xlsx` | Путь для восстановленного XLSX. **Обязательно**. |
| `--map` | - | `mapping.json` | Mapping JSON, созданный командой `mask`. **Обязательно**. |
